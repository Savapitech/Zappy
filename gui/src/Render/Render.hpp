...
namespace Zappy {
    ...
    public:
        Renderer(unsigned int width, unsigned int height) : _width(width), _height(height) {
        ...
        void render(const Camera& camera, InstancedGrid& floor, const std::vector<std::unique_ptr<Sprite>>& players) {
            ...
            Zappy::Math::mat4 projection = Zappy::Math::perspective(Zappy::Math::radians(45.0f), static_cast<float>(_width) / static_cast<float>(_height), 0.1f, 1000.0f);
