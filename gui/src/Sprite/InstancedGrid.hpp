    ...
    public:
        InstancedGrid(Texture& texture) : _texture(texture) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                glVertexAttribPointer(2 + i, 4, GL_FLOAT, GL_FALSE, 16 * sizeof(float), (void*)(i * vec4Size));
        ...
        void build() {
            glBindVertexArray(_VAO);
            glBindBuffer(GL_ARRAY_BUFFER, _instanceVBO);
            glBufferData(GL_ARRAY_BUFFER, _matrices.size() * 16 * sizeof(float), _matrices[0].value_ptr(), GL_STATIC_DRAW);
            glBindVertexArray(0);
        }
        ...
        void draw(Shader& shader, const Zappy::Math::mat4& viewProj) {
            ...
            glBindVertexArray(_VAO);
            glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0, _matrices.size());
            glBindVertexArray(0);
        }
    };
}