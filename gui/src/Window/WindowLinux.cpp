...
void Window::open(unsigned int width, unsigned int height,
  ...
  GLXFBConfig *fbc =
      glXChooseFBConfig(dpy, DefaultScreen(dpy), visualAttribs, &fbcount);
  if (!fbc) {
  ...
  std::unique_ptr<XVisualInfo, XDeleter> vi(
      glXGetVisualFromFBConfig(dpy, bestFbc));
  ...
  ::Window win = XCreateWindow(dpy, RootWindow(dpy, vi->screen), 0, 0, width,
  ...
  GLXContext ctx =
      glXCreateContextAttribsARB(dpy, bestFbc, 0, True, contextAttribs);
  _context = ctx;
  ...
  glXSwapIntervalEXTProc glXSwapIntervalEXT =
      (glXSwapIntervalEXTProc)glXGetProcAddressARB(
...
const std::vector<Zappy::Event> &Window::pollEvents() {
  ...
  while (XPending(dpy) > 0) {
    ...
    switch (xev.type) {
    case ClientMessage:
      if (static_cast<unsigned long>(xev.xclient.data.l[0]) ==
          _wmDeleteMessage) {
