...
#include <iostream>
namespace Zappy {
Button::Button(Texture &texture, float x, float y, float width, float height,
               std::function<void()> function, WindowSize &ws)
    : _function(function), _width(width), _height(height), _x(x), _y(y),
      _hovered(false), _ws(ws) {
...
void Button::draw(Shader &shader) {
  if (_sprite) {
    Zappy::Math::mat4 orthoProjection =
        Zappy::Math::ortho(0.0f, _ws.width, _ws.height, 0.0f, -1.0f, 1.0f);
...
void Button::update(const std::vector<Zappy::Event> &events) {
  for (const auto &event : events) {
    if (event.type == EventType::MouseMoved) {
      if (event.mouseX >= _x - _width / 2 && event.mouseX <= _x + _width / 2 &&
          event.mouseY >= _y - _height / 2 &&
          event.mouseY <= _y + _height / 2) {
        _hovered = true;
      } else {
        _hovered = false;
