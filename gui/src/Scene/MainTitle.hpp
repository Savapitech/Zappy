...
private:
  ...
  float randFloat(float min, float max) {
    float random = min + static_cast<float>(rand()) /
...
public:
  MainTitle(TextureManager &tm, audioManager &audioManager)
      : _texManager(tm), _audios(audioManager) {}
  void onEnter() override {
    ...
    Font &titleFont =
        _fontManager.get("gui/assets/fonts/mainTitle.otf", 256.0f, 4096);
    ...
    _pressStartText = std::make_unique<Text>(
        startFont, "Press ANY KEY to start", 0.0f, 200.0f);
    ...
    glBufferData(GL_ARRAY_BUFFER, _particles.size() * 6 * sizeof(float),
      ...
      if (!_isExiting) {
        for (const auto &event : events) {
          if (event.type == Zappy::EventType::KeyPressed ||
              event.type == Zappy::EventType::MousePressed) {
            _isExiting = true;
            break;
          }
        }
  ...
  void draw(Shader &, WindowSize &windowSize) override {
    ...
    Zappy::Math::mat4 orthoProjection =
    ...
    if (_zoomFinished) {
      ...
      for (const auto &p : _particles) {
        pData.push_back(p.position.x);
        pData.push_back(p.position.y);
        pData.push_back(p.color.x);
        pData.push_back(p.color.y);
        pData.push_back(p.color.z);
