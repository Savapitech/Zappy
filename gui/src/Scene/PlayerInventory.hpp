...
public:
  playerInventory(TextureManager &tm, Zappy::NetworkManager &nm,
                  FontManager &fm)
      : _texManager(tm), _networkManager(nm), _fontManager(fm) {}
  void onEnter() override {
    Texture &playerInventoryTex =
        _texManager.get("gui/assets/tileInventory.png");
    ...
    for (int i = 0; i < 7; i++) {
      ...
      auto txt = std::make_unique<Text>(font, "0", fixedX + 60.0f,
                                        startY + (i * spacing) + 40.0f);
  ...
  void draw(Shader &shader, WindowSize &windowSize) override {
    if (_targetPlayerId == -1 || !_playerInventorySprite || !_uiShader ||
        !_textShader)
      return;
