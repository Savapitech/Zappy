...
namespace Zappy {
Font::Font(const std::string &filepath, int atlasWidth, int atlasHeight,
           float fontSize)
    : _id(0), _atlasWidth(atlasWidth), _atlasHeight(atlasHeight),
      _fontSize(fontSize) {
  ...
  stbtt_BakeFontBitmap(ttfBuffer.data(), 0, _fontSize, tempBitMap.data(),
                       _atlasWidth, _atlasHeight, 32, 96, _charData);
  glGenTextures(1, &_id);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, _atlasWidth, _atlasHeight, 0, GL_RED,
               GL_UNSIGNED_BYTE, tempBitMap.data());
