...
public:
  ...
  Zappy::Math::vec3 getFront() const {
    ...
    front.x = std::cos(Zappy::Math::radians(yaw)) * std::cos(Zappy::Math::radians(pitch));
    front.y = std::sin(Zappy::Math::radians(pitch));
    front.z = std::sin(Zappy::Math::radians(yaw)) * std::cos(Zappy::Math::radians(pitch));
    return Zappy::Math::normalize(front);
  }

  Zappy::Math::mat4 getViewMatrix() const {
    Zappy::Math::vec3 front = getFront();
    Zappy::Math::vec3 target = {
        position.x + front.x, 
        position.y + front.y, 
        position.z + front.z
    };
