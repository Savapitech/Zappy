...
public:
  quickMenu(TextureManager &tm, Zappy::NetworkManager &nm, WindowSize &ws)
      : _texManager(tm), _networkManager(nm), _windowSize(ws) {
    _speed = 0;
  }
  void onEnter() override {
    ...
    _uiShader = std::make_unique<Shader>("gui/src/Core/Shader/ui.vert",
    ...
    auto helper = []() {
      std::cout
          << "You can decrease / increase the number of ticks per seconds."
          << std::endl;
    };
    auto decrease = [this]() {
    ...
    _SpeedButton = std::make_unique<Zappy::Button>(
        speedButtonTex, (_windowSize.width / 2.0f) - 40.0f,
        (_windowSize.height / 2.0f) - 40.0f, 80.0f, 80.0f, helper, _windowSize);
    _IncreaseButton =
        std::make_unique<Zappy::Button>(increaseButtonTex, 1050.0f, 465.0f,
                                        50.0f, 50.0f, increase, _windowSize);
    _DecreaseButton = std::make_unique<Zappy::Button>(
        decreaseButtonTex, 850.0f, 465.0f, 50.0f, 50.0f, decrease, _windowSize);
  ...
  SceneState update(const std::vector<Zappy::Event> &events,
    ...
    _backgroundSprite->setPosition(_windowSize.width / 2.0f,
                                   (_windowSize.height - 415.0f) / 2.0f);
  ...
  void draw(Shader &shader, WindowSize &windowSize) override {
    ...
    _uiShader->bind();
    Zappy::Math::mat4 orthoProjection = Zappy::Math::ortho(
